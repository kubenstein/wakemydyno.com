class BackgroundProcessSpawnerService
  ...
  def self.spawn_ping_process(initial_sleep = 20.seconds, loop_condition = -> { true }, inloop_sleep = 20.minutes)
    sleep initial_sleep
  ...
  def self.spawn_url_check_process(initial_sleep = 24.hours - Time.now.seconds_since_midnight.to_i, loop_condition = -> { true }, inloop_sleep = 24.hours)
