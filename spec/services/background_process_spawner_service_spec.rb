...
describe BackgroundProcessSpawnerService do
  ...
  context 'pinging process' do
    it 'should ping in the loop' do
      ...
      BackgroundProcessSpawnerService.spawn_ping_process(initial_sleep = 0.seconds,
    ...
    it 'should wait initial time' do
      start_time = Time.now
      BackgroundProcessSpawnerService.spawn_ping_process(initial_sleep = 2.seconds,
  ...
  context 'checking process' do
    it 'should check in the loop' do
      ...
      BackgroundProcessSpawnerService.spawn_url_check_process(initial_sleep = 0.seconds,
                                                              loop_condition = -> { (loop_runs = loop_runs -1) > -1 },
    ...
    it 'should wait initial time' do
      start_time = Time.now
      BackgroundProcessSpawnerService.spawn_url_check_process(initial_sleep = 2.seconds,
                                                              loop_condition = -> { false },
                                                              inloop_sleep = 0.hours)
      end_time = Time.now
      expect((end_time - start_time).to_i).to eq 2
