...
describe BackgroundProcessesService do
  ...
  context 'pinging process' do
    it 'should ping in the loop' do
      ...
      BackgroundProcessesService.start_pinging_process(initial_sleep = 0.seconds,
    ...
    it 'should wait initial time' do
      start_time = Time.now
      BackgroundProcessesService.start_pinging_process(initial_sleep = 2.seconds,
  ...
  context 'checking process' do
    it 'should check in the loop' do
      ...
      BackgroundProcessesService.start_urls_checking_process(initial_sleep = 0.seconds,
                                                            loop_condition = -> { (loop_runs = loop_runs -1) > -1 },
    ...
    it 'should wait initial time' do
      start_time = Time.now
      BackgroundProcessesService.start_urls_checking_process(initial_sleep = 2.seconds,
                                                            loop_condition = -> { false },
                                                            inloop_sleep = 0.hours)
      end_time = Time.now
      expect((end_time - start_time).to_i).to eq 2
